...
public class SymbolScope implements Xferable {
	...
	public static Schema schema = Schema.getInstance();

	protected ArrayList<Symbol> allSymbols = new ArrayList<Symbol>();
	protected ArrayList<Call> callList = new ArrayList<Call>();
	protected ArrayList<SymbolScope> childScopes = new ArrayList<SymbolScope>();
	 ...
	 * Note that this isn't really complete. It's possible to have an IN SUPER
	 * declaration, as well as a local definition. The local definition should
	 * be the one in this map, but as it stands, the *last added* is what will be found.
	 */
	 ...
	 * This is called when copying a SymbolScopeSuper's symbols for inheritance purposes.
	 */
	...
	public <T extends Symbol> ArrayList<T> getAllSymbolsDeep(Class<T> klass) {
		ArrayList<T> ret = getAllSymbols(klass);
		for (SymbolScope child : childScopes) {
	...
	public ArrayList<Call> getCallList() { return callList; }
	...
	public void registerCall(Call call) { callList.add(call); }
